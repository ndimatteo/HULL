...
export function useFilters(fallback) {
  const router = useRouter()
  const currentPath = [].concat(router.query?.slug).join('/')
  const hasQuery = Object.keys(router.query).length
  let currentFilters = fallback

  // if query params present, update the current filters
  if (hasQuery) {
    currentFilters = fallback.map((filter) =>
      router.query[filter.name]
        ? { ...filter, value: router.query[filter.name] }
        : filter
    )
  }
      ...
      router.replace(
        {
          ...
          query: filters
            .filter(
              (f) => f.value !== fallback.find((fb) => fb.name === f.name).value
            )
            .reduce((r, { name, value }) => ((r[name] = value), r), {}),
        },
        undefined,
        {
          shallow: true,
