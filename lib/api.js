...
export async function getProduct(slug, variant = false) {
  const data = await sanity.fetch(
    `
    {
      "product": *[_type == "product" && slug.current == $slug && wasDeleted != false][0]{
        "id": productID,
        "slug": slug.current,
        title,
        price,
        "variants": *[_type == 'productVariant' && productID == ^.productID && wasDeleted != false]{
          "id": variantID,
          title,
          price
        },
        "activeVariant": ${
          variant
            ? '*[_type == "productVariant" && variantTitle match $variant && wasDeleted != false][0].variantID'
            : 'variantID'
        },
        seo
      },
